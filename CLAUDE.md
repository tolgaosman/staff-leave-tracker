...
- **Endpoint surface:** `POST /login` (rate-limited), `POST /logout`, `POST /register`, `GET /me`, `PUT /me`, `GET /dashboard`, `GET /departments`, `GET /leave-types`, `GET /personnel`, `GET /personnel/:id`, `POST /personnel`, `PUT /personnel/:id`, `DELETE /personnel/:id`, `GET /leave-requests`, `POST /leave-requests`, `PUT /leave-requests/:id`, `PATCH /leave-requests/:id/approve`, `PATCH /leave-requests/:id/reject` (body `{ rejection_reason }`), `DELETE /leave-requests/:id`.
